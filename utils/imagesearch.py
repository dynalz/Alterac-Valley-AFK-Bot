...
def click_image(image, pos, action, timestamp):
    img = cv2.imread(image)
    height, width, channels = img.shape
    pyautogui.moveTo(pos[0] + r(width / 2, width/3), pos[1] + r(height/2, height/3),
                     timestamp)
    pyautogui.click(button=action)


def get_random_position_in_img(image, pos):
    img = cv2.imread(image)
    height, width, channels = img.shape
    return (pos[0] + r(width / 2, width/3), pos[1] + r(height/2, height/3))
...
def imagesearch(image, precision=0.7):
    ...
    min_val, max_val, min_loc, max_loc = cv2.minMaxLoc(res)
    if max_val < precision:
        #print("Precision on {} = {}".format(image, max_val))
        return None
    return max_loc
